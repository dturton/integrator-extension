...
Extension.prototype.callFunction = function (functionOptions, extensionOptions, callback) {
  ...
  if (errorList.length > 0) return callback({
    statusCode: 422,
    errors: errorList
  })
  ...
  var object = module[util.getObjectName(.type)]
  if (!object) {
    return callback({
      statusCode: 422,
      errors: [{
        message: 'The object ' + extensionOptions.type + " doesn't exist in the module.",
  ...
  if (!util.isFunction(object[functionName])) {
    return callback({
      statusCode: 422,
      errors: [errors.get('EXTENSION_INCORRECT_FUNCTION')]
    })
  }
  ...
  object[functionName](functionOptions, function (err, result) {
    ...
    if (responseValidationError) return callback({
      statusCode: 422,
